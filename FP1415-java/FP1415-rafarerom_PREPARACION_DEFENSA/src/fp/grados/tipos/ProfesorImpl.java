...
//	}
	...
	public void imparteAsignatura(Asignatura a, Double d) {
		...
		if(asignaturas.contains(a)){
			checkDedicacionTotal(a, d);
			int cont= asignaturas.indexOf(a);
			creditos.set(cont, d);
		}
	...
	private void checkDedicacion(Asignatura a, Double d) {
		if (d > a.getCreditos() || d <= 0) {
			throw new ExcepcionProfesorOperacionNoPermitida();
		}

	}
	
	private void checkDedicacionTotal (Asignatura a, Double dedicacion){
		Double dedica=dedicacionAsignatura(a);
		Double dedicaciontotal=getDedicacionTotal();
		dedicaciontotal=dedicaciontotal-dedica+dedicacion;
		if(dedicaciontotal >CREDITOS_MAXIMO ){
			throw new ExcepcionProfesorOperacionNoPermitida();
		}
	}
